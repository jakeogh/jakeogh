...
src_configure() {
	...
	# Top-level shim: force system deps; no FetchContent downloads.
...
# Create BOTH names Datoviz may reference:
#   - msdf-atlas-gen::msdf-atlas-gen  (what it links to)
#   - msdfgen-atlas                   (if scripts refer to the plain target)
function(_provide_msdf_targets_from_components)
  # Use whatever components the system package exposes
  ...
  # link interface to components/real libs
...
function(_ensure_msdfgen)
  # If either target already exists, we're done.
  if(TARGET msdf-atlas-gen::msdf-atlas-gen OR TARGET msdfgen-atlas)
    return()
  endif()

  # Prefer system CMake package
  find_package(msdfgen CONFIG QUIET)
  if(TARGET msdfgen::msdfgen-core OR TARGET msdfgen::msdfgen-ext OR TARGET msdfgen::msdfgen)
    _provide_msdf_targets_from_components()
    return()
  endif()

  # Fallback: locate split libs directly
